...
class SBI_Bank extends Bank
{
	...
	void calculation()
	{
		amount = deposit*Math.pow((1 + (get_rate_of_interest()/100)),time_period);
		System.out.printf("The amount at the end will be %.2f\n",amount);
	}
}
class ICICI_Bank extends Bank
{
    double deposit = 12500;
...
class AXIS_Bank extends Bank
{
	...
	void calculation()
	{
		amount = deposit*Math.pow((1 + (get_rate_of_interest()/100)),time_period/365);
		System.out.printf("The amount at the end will be %.2f\n",amount);
	}
}
public class Expt6_1
{
	public static void main(String[] args)
	{
		...
		System.out.println("Day of deposition: "+cal.getTime());
		cal.add(Calendar.DATE,t);
		System.out.println("Maturity Date: "+cal.getTime());
		switch(i)
		{
			case 1:
